...
def _compare_tables(
    ...
    tname_to_table = {no_dflt_schema: autogen_context.table_key_to_table[
...
def _compare_columns(
    ...
    conn_col_names = {c.name: c for c in conn_table.c
            if autogen_context.run_name_filters(
...
def _correct_for_uq_duplicates_uix(
    ...
    uqs_dupe_indexes = {cons.name: cons for cons in conn_unique_constraints
            if cons.info["duplicates_index"]}

    for overlap, value in uqs_dupe_indexes.items():
        if overlap not in metadata_uq_names:
            if (
                ...
                conn_unique_constraints.discard(value)
...
def _setup_autoincrement(
    ...
    if (
        metadata_col.table._autoincrement_column is metadata_col
        or metadata_col.autoincrement is True
    ):
...
def _render_server_default_for_compare(
    ...
    if isinstance(metadata_default, compat.string_types):
        if metadata_col.type._type_affinity is not sqltypes.String:
...
def _compare_table_comment(
    ...
    if metadata_table.comment is None:
        modify_table_ops.ops.append(

...
class RevisionMap(object):
    ...
    def _revision_for_ident(self, resolved_id, check_branch=None):
        branch_rev = self._resolve_branch(check_branch) if check_branch else None
        try:
        ...
        if (
            ...
            and not self._shares_lineage(revision.revision, branch_rev.revision)
        ):
            raise ResolutionError(
    ...
    def _relative_iterate(
        ...
        if not isinstance(destination, compat.string_types):
            return None
        ...
        if is_upwards:
            if branch_label:
                ...
                from_ = symbol if symbol.startswith("head") else "%s@head" % symbol
            else:
                from_ = "head"
            to_ = source
        else:
    ...
    def _iterate_revisions(
        ...
        # member nodes
        branch_todo = {rev for rev in (self._revision_map[rev] for rev in total_space)

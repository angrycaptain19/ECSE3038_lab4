...
class ApplyBatchImpl(object):
    ...
    def _setup_referent(self, metadata, constraint):
        ...
        parts = spec.split(".")
        tname = parts[-2]
        referent_schema = parts[0] if len(parts) == 3 else None
    ...
    def _setup_dependencies_for_add_column(
        ...
        if not self.partial_reordering:
            if insert_after and not insert_before:
            ...
            if insert_before and not insert_after:
    ...
    def add_constraint(self, const):
        ...
        if (
            isinstance(const, sql_schema.PrimaryKeyConstraint)
            and self.table.primary_key in self.unnamed_constraints
        ):
